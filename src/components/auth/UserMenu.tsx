    ...
    <>
      {!user && (
        <button
          ...
          onClick={!isSigningIn ? onGithubSignIn : undefined}
        >
          {isSigningIn ? <LoaderIcon className="text-xl" /> : "Sign In"}
        </button>
      )}
      {user && (
        <>
          ...
          <DropdownMenu.Root>
            ...
            <DropdownMenu.Portal>
              <DropdownMenu.Content className="min-w-[200px] bg-white dark:bg-gray-800 rounded-lg shadow-lg p-2 z-50">
                ...
                <DropdownMenu.Item
                  className="px-3 py-2 outline-none text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded cursor-pointer"
